  ...
  switch (message.GetMessageType ())
    {
      ...
      // -------- Milestone 1: Stabilization / Notify ----------
      case PennChordMessage::STABILIZE_REQ:
        // successor asking this node for its predecessor
        ProcessStabilizeReq(message);
        break;

      case PennChordMessage::STABILIZE_RSP:
        // response from successor with its predecessor
        ProcessStabilizeRsp(message);
        break;

      case PennChordMessage::NOTIFY_MSG:
        // some node telling us it might be our predecessor
        ProcessNotifyMsg(message);
        break;

      // -------- Milestone 2A: Lookup Messages ----------
...
{
  // check if node has no successor set
  if (m_successor == Ipv4Address::GetAny ()) return;

  // send a request to successor for its predessor
...
// FIX: Data transfer logic added for joining node
void PennChord::Notify(Ipv4Address potentialPred)
{
  ...
  if (m_predecessor == Ipv4Address::GetAny() ||
      IsBetween(potentialPred, m_predecessor, GetLocalAddress()))
    {
      ...
      // This node must acquire keys from its successor that now fall in (new_pred, self]
      if (oldPredecessor != m_predecessor && oldPredecessor != Ipv4Address::GetAny())
      {
          // Signal the successor (m_successor) to transfer keys that fall in (old_pred, potentialPred]
          // to the new predecessor (potentialPred).
          // Simplification: We signal the app layer that we have a new predecessor.
