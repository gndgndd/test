...
/* ============================================================================
   MS2A: START SEARCH (USER INITIATED, LOCAL NODE)
   This simply determines the origin IP for a search initiated on *this* node
   and delegates to StartSearchFromOrigin.
   ============================================================================ */
void
PennSearch::StartSearch (const std::vector<std::string> &terms)
{
  ...
  ss << GetLocalAddress ();
  std::string originIp = ss.str ();
  StartSearchFromOrigin (terms, originIp);
}

/* ============================================================================
   Helper: StartSearchFromOrigin
   - Used both for a normal local search (origin is this node)
   - And when a via node starts the search on behalf of some other originIp.
   The algorithm is identical to the original StartSearch, except that
   originIp is provided as a parameter.
   ============================================================================ */
void
PennSearch::StartSearchFromOrigin (const std::vector<std::string> &terms,
