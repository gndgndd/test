...
uint32_t 
PennSearchMessage::SearchReq::GetSerializedSize() const 
{
...
uint32_t 
PennSearchMessage::SearchRsp::GetSerializedSize() const 
{
...
void
PennSearchMessage::SetPublishReq (std::string keyword, std::vector<std::string> docID)
{
  if (m_messageType == 0)
    {
...
uint32_t 
PennSearchMessage::RejoinReq::GetSerializedSize() const 
{
  return IPV4_ADDRESS_SIZE;
}

void 
PennSearchMessage::RejoinReq::Print(std::ostream &os) const 
{
  os << "Rejoin - requester = " << requestor;
}

void 
PennSearchMessage::RejoinReq::Serialize(Buffer::Iterator &start) const 
{
  uint32_t ip = requestor.Get();
  start.WriteHtonU32(ip);
}

uint32_t 
PennSearchMessage::RejoinReq::Deserialize(Buffer::Iterator &start) 
{
  requestor = Ipv4Address(start.ReadNtohU32());
  return GetSerializedSize();
}
...
PennSearchMessage::SetRejoinReq (Ipv4Address requesterIp)
{
  ...
  m_message.rejoinReq.requestor = requesterIp;
}
