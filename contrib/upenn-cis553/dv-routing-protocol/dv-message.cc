...
/* ---- HELLO_REQ ---- */
uint32_t DVMessage::HelloReq::GetSerializedSize() const { return sizeof(uint16_t) + helloMessage.length(); }
void DVMessage::HelloReq::Print (std::ostream &os) const { os << "    HelloReq msg='" << helloMessage << "'\n"; }
void DVMessage::HelloReq::Serialize (Buffer::Iterator &it) const
{
  it.WriteU16(helloMessage.length());
  it.Write(reinterpret_cast<const uint8_t*>(helloMessage.c_str()), helloMessage.length());
}
uint32_t DVMessage::HelloReq::Deserialize (Buffer::Iterator &it)
{
  uint16_t n = it.ReadU16();
  std::string s; s.resize(n);
  it.Read(reinterpret_cast<uint8_t*>(&s[0]), n);
  helloMessage = s;
...
void DVMessage::HelloRsp::Serialize (Buffer::Iterator &it) const
{
  it.WriteHtonU32(sourceAddress.Get());
  it.WriteU16(helloMessage.length());
  it.Write(reinterpret_cast<const uint8_t*>(helloMessage.c_str()), helloMessage.length());
}
uint32_t DVMessage::HelloRsp::Deserialize (Buffer::Iterator &it)
{
  sourceAddress = Ipv4Address(it.ReadNtohU32());
  uint16_t n = it.ReadU16();
  std::string s; s.resize(n);
  it.Read(reinterpret_cast<uint8_t*>(&s[0]), n);
  helloMessage = s;
...
void DVMessage::PingReq::Serialize (Buffer::Iterator &it) const
{
  it.WriteHtonU32(destinationAddress.Get());
  it.WriteU16(pingMessage.length());
  it.Write(reinterpret_cast<const uint8_t*>(pingMessage.c_str()), pingMessage.length());
}
uint32_t DVMessage::PingReq::Deserialize (Buffer::Iterator &it)
{
  destinationAddress = Ipv4Address(it.ReadNtohU32());
  uint16_t n = it.ReadU16();
  std::string s; s.resize(n);
  it.Read(reinterpret_cast<uint8_t*>(&s[0]), n);
  pingMessage = s;
...
void DVMessage::PingRsp::Serialize (Buffer::Iterator &it) const
{
  it.WriteHtonU32(destinationAddress.Get());
  it.WriteU16(pingMessage.length());
  it.Write(reinterpret_cast<const uint8_t*>(pingMessage.c_str()), pingMessage.length());
}
uint32_t DVMessage::PingRsp::Deserialize (Buffer::Iterator &it)
{
  destinationAddress = Ipv4Address(it.ReadNtohU32());
  uint16_t n = it.ReadU16();
  std::string s; s.resize(n);
...
{
  os << "    DV vector items=" << vec.size() << "\n";
  for (const auto &it : vec) os << "      dest=" << it.dest << " cost=" << it.cost << "\n";
}
...
{
  it.WriteU16(static_cast<uint16_t>(vec.size()));
  for (const auto &v : vec) { it.WriteHtonU32(v.dest.Get()); it.WriteHtonU32(v.cost); }
}
uint32_t DVMessage::DvUpdate::Deserialize (Buffer::Iterator &it)
{
  ...
  for (uint16_t i=0; i<n; ++i) {
    DvVectorItem item;
    item.dest = Ipv4Address(it.ReadNtohU32());
...
/* common header mutators/accessors */
...
void DVMessage::SetTTL (uint8_t ttl) { m_ttl = ttl; }
