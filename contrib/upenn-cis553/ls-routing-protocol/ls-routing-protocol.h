...
private:
  ...
  struct LsaRecord {
    ...
    std::vector<std::pair<uint32_t, uint32_t>> links; // (neighborId, cost)
  };
  ...
  std::map<uint32_t, AdjacencyRecord> m_neighbors;          // nodeId -> adjacency
  std::map<uint32_t, LsaRecord>       m_linkStateDatabase;  // originatorId -> LSA
  std::map<uint32_t, RouteRow>        m_routingTable;       // destId -> route
};
