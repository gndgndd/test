...
LSMessage::Deserialize(Buffer::Iterator start)
{
  ...
  case HELLO_RSP:
    ...
    break;
  case LSA_m:
    size += m_message.lsa.Deserialize(i);
    break;
  default:
    NS_ASSERT(false);
  }
...
/* PING_REQ and HELLO_REQ/RSP functions refactored for reusability */

void SerializeStringPayload(Buffer::Iterator &start, const Ipv4Address &destAddr, const std::string &message)
{
...
uint32_t DeserializeStringPayload(Buffer::Iterator &start, Ipv4Address &destAddr, std::string &message)
{
...
/* LSA_m functions refactored for clarity and uniqueness */
uint32_t
LSMessage::Lsa::GetSerializedSize(void) const
{
  uint32_t size = sizeof(uint16_t);
  size += (sizeof(uint32_t) + sizeof(uint32_t)) * linkVector.size();
  return size;
}
void
...
{
  os << "Lsa:: Links: ";
  for (const auto& link : linkVector) {
...
{
  start.WriteU16(linkVector.size());
  for (const auto& link : linkVector) {
    start.WriteHtonU32(link.first);
    start.WriteHtonU32(link.second);
  }
}
uint32_t
LSMessage::Lsa::Deserialize(Buffer::Iterator &start)
{
  ...
  for (uint16_t i = 0; i < count; ++i) {
    ...
    uint32_t linkCost = start.ReadNtohU32();
    linkVector.push_back(std::make_pair(nodeNum, linkCost));
  }
  return Lsa::GetSerializedSize();
}

/* Set and Get functions for new message types */
...
void
LSMessage::SetLsa(const std::vector<std::pair<uint32_t, uint32_t>>& links)
{
  if (m_messageType == 0) {
    m_messageType = LSA_m;
  } else {
  ...
  m_message.lsa.linkVector = links;
}
LSMessage::Lsa
LSMessage::GetLsa()
{
...
/* Below are functions not touched or mirrored */
void
