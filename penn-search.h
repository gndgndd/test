...
class PennSearch : public PennApplication
{
  public:
    ...
    // User-initiated search entry point (for local node)
  ...
  private:
    ...
    // Helper: start a search where the logical origin may be a *different* node
    // (used when a non-Chord node issues SEARCH via some Chord node).
    void StartSearchFromOrigin (const std::vector<std::string> &terms,
                                const std::string &originIp);
};
